<?php
  ...
  class Oahu_Connection {
    ...
    private function consumerSignature() {
      $sig_time   = time();
      $signature  = md5(implode("-", array($sig_time, $this->clientId, $this->appSecret)));
      return implode("|", array($sig_time, $this->clientId, $signature));
    }
    ...
    public function exec($type, $path, $params = array(), $headers = array()) {
      ...
      $url = $this->host . "/api/v1/clients/" . $this->clientId . "/" . $path;
